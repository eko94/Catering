...
{
    public class EmpaquetarOrdenHandlerTest
    {
        ...
        public EmpaquetarOrdenHandlerTest()
        {
        ...
        [Fact]
        public async Task EmpaquetarOrdenHandler()
        {
            // Arrange
            ...
            _ordenTrabajoRepository.Verify(x => x.GetByIdAsync(ordenTrabajo.Id, false), Times.Exactly(3));
            _ordenTrabajoRepository.Verify(x => x.UpdateAsync(ordenTrabajo), Times.Exactly(3));

            _unitOfWork.Verify(x => x.CommitAsync(tcs.Token), Times.Exactly(4));
        }
    }
}
